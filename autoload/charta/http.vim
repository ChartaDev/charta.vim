" http
" Kindly stolen from:
" https://github.com/mattn/webapi-vim/blob/master/autoload/webapi/http.vim
let s:save_cpo = &cpo
set cpo&vim
...
function! s:nr2byte(nr) abort
  ...
  elseif a:nr < 0x4000000
    ...
    return nr2char(a:nr/1073741824%16+252).nr2char(a:nr/16777216%16+128).nr2char(a:nr/262144%16128).nr2char(a:nr/4096/16+128).nr2char(a:nr/64%64+128).nr2char(a:nr%64+128)
  endif
endfunction
...
function! charta#http#decodeURI(str) abort
  let ret = a:str
...
function! charta#http#escape(str) abort
  return substitute(a:str, '[^a-zA-Z0-9_.~/-]', '\=s:urlencode_char(submatch(0))', 'g')
endfunction

function! charta#http#encodeURI(items, ...) abort
  let is_binary = get(a:000, 1)
  let ret = ''
  if type(a:items) == 4
    for key in sort(keys(a:items))
      if strlen(ret) | let ret .= '&' | endif
      let ret .= key . '=' . charta#http#encodeURI(a:items[key])
    endfor
...
function! charta#http#encodeURIComponent(items) abort
  let ret = ''
  if type(a:items) == 4
    for key in sort(keys(a:items))
      if strlen(ret) | let ret .= '&' | endif
      let ret .= key . '=' . charta#http#encodeURIComponent(a:items[key])
    endfor
  elseif type(a:items) == 3
    for item in sort(a:items)
      if strlen(ret) | let ret .= '&' | endif
...
function! charta#http#get(url, ...) abort
  let getdata = a:0 > 0 ? a:000[0] : {}
  let headdata = a:0 > 1 ? a:000[1] : {}
  let follow = a:0 > 2 ? a:000[2] : 1
  let url = a:url
  let getdatastr = charta#http#encodeURI(getdata)
...
function! charta#http#post(url, ...) abort
  ...
  let url = a:url
  if type(postdata) == 4
    let postdatastr = charta#http#encodeURI(postdata)
  else
...
function! charta#http#send(req) abort
...
function! charta#http#stream(req) abort
  let postdata = get(a:req, 'data', '')
  let method = get(a:req, 'method', postdata == '' ? 'GET': 'POST')
  let headdata = get(a:req, 'header', {})
  let follow = get(a:req, 'follow', 1)
  ...
  \{
  \  'exit_cb': function('charta#http#exit_cb', [a:req]),
  \  'stoponexit': 'kill',
  \})
  let a:req['job'] = job
  ...
  \{
  \  'out_cb': function('charta#http#out_cb', [a:req]),
  \  'mode': mode,
  \})
...
function! charta#http#exit_cb(req, job, code) abort
...
function! charta#http#out_cb(req, ch, data) abort
